...
export class DetailclientComponent {
  ...
  OnSearchPieceKeyUp(event) {
    ...
    this.filtredClientPieces = this.currentClient.ClientPieces.filter(
      (x) =>
  ...
  dialogImportPiece: any = {
    ...
    formData: null,
    Open: (content) => {
      this.dialogImportPiece.PiecesToChoose = this.Pieces.filter(
        (x) =>
          !this.currentClient.ClientPieces.map((w) => w.PieceId).includes(
            x.PieceId
          )
      );
    ...
    Submit: () => {
      if (
        this.dialogImportPiece.SelectedPiece == null ||
        this.dialogImportPiece.SelectedPiece == ""
      ) {
      ...
      this.dialogImportPiece.formData.append(
        "ClientPieceId",
        newClientPiece.ClientPieceId
      );
      this.dialogImportPiece.formData.append(
        "ClientId",
        newClientPiece.ClientId
      );
      this.dialogImportPiece.formData.append("PieceId", newClientPiece.PieceId);

      this.loader.show();
      this.clientService
        .CreateClientPiece(this.dialogImportPiece.formData)
        .subscribe(
          (response: any) => {
